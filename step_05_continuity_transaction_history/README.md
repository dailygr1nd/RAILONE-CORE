# RailOne Pilot Convergence - Step 05
...
operational ContUID lookup, append-only RIV history, permissioned UTT/ContUID
transaction views, and the first durable PostgreSQL schema contract.
...
```powershell
python .\step_05_continuity_transaction_history\run_tests.py
```
...
Step 06 should implement the PostgreSQL adapters, authenticated transaction
history API, transactional outbox, signed projection events, and controlled
reconciliation release before any live-value pilot.
