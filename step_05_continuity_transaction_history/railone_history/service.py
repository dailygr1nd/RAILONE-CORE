...
class TransactionHistoryService:
    ...
    def list_by_continuity_uid(
        ...
        *,
        ...
        allowed = self._can_access(SubjectKind.CONTINUITY_UID, normalized, access)
        self._audit(
            access=access,
            target_kind=SubjectKind.CONTINUITY_UID.value,
        ...
        if not allowed:
            raise TransactionAccessDeniedError(
                "principal cannot read this continuity transaction history"
            )
        rows = list(self._history.list_for_subject(SubjectKind.CONTINUITY_UID, normalized))
