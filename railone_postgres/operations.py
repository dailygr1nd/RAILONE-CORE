...
rejection_disposition, provider_context, version, created_at, updated_at"""
...
class PostgresOperationsStore(OperationsStore):
    ...
    def find_by_external_reference(
        self, provider_id: str, external_reference: str
    ) -> ProviderSubmissionRecord | None:
        with self._database.transaction() as connection, connection.cursor() as cursor:
            cursor.execute(
                f"""SELECT {_SUBMISSION_COLUMNS}
                    FROM railone.provider_submissions
                    WHERE provider_id=%s AND external_reference=%s""",
            ...
            cursor.execute(
                """UPDATE railone.provider_submissions
                   SET state=%s, dispatch_attempts=%s, normalized_code=%s,
                       ...
                       provider_context=%s::jsonb, version=%s, updated_at=%s
                ...
                (
                    ...
                    json_text(dict(submission.provider_context)),
...
def _submission_params(value: ProviderSubmissionRecord) -> tuple[object, ...]:
    return (
        ...
        json_text(dict(value.provider_context)),
...
def _submission(row: Mapping[str, Any]) -> ProviderSubmissionRecord:
    ...
    return ProviderSubmissionRecord(
        ...
        provider_context=tuple(
            sorted(
                (str(key), str(item))
                for key, item in json_object(row["provider_context"]).items()
            )
        ),
