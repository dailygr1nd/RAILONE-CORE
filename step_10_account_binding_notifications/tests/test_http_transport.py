...
class HttpTransportTests(unittest.TestCase):
    ...
    def test_mpesa_callback_route_is_available_only_when_processor_is_wired(self):
        class StubCallbacks:
            def process_result(self, *, raw_body, ingress_signature):
                self.received = (raw_body, ingress_signature)
                return {"disposition": "TEST_ACCEPTED"}

        callbacks = StubCallbacks()
        client = self.TestClient(
            self.create_app(facade=self.facade, mpesa_callbacks=callbacks)
        )
        response = client.post(
            "/v1/provider-callbacks/mpesa/result",
            content=b'{"Result":{}}',
            headers={"X-RailOne-Ingress-Signature": "sha256=test"},
        )

        self.assertEqual(response.status_code, 202)
        self.assertEqual(callbacks.received, (b'{"Result":{}}', "sha256=test"))
