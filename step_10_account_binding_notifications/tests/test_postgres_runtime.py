...
class PostgresRuntimeTests(unittest.TestCase):
    ...
    def test_step_10_migration_has_opaque_bindings_and_sms_outbox_guards(self):
        root = Path(__file__).resolve().parents[1]
        sql = (root / "migrations" / "0006_partner_bindings_settlement_notifications.sql").read_text()
        self.assertIn("account_bindings", sql)
        self.assertIn("raw account, wallet, card and MSISDN endpoints are prohibited", sql)
        self.assertIn("settlement_evidence_append_only", sql)
