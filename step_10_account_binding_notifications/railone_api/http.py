...
from railone_callbacks import MpesaCallbackProcessor
...
def create_app(
    *,
    ...
    mpesa_callbacks: MpesaCallbackProcessor | None = None,
) -> FastAPI:
    app = FastAPI(
    ...
    if mpesa_callbacks is not None:
        @app.post("/v1/provider-callbacks/mpesa/result", status_code=202)
        async def mpesa_result(
            request: Request,
            ingress_signature: str = Header(
                default="", alias="X-RailOne-Ingress-Signature"
            ),
        ):
            result = mpesa_callbacks.process_result(
                raw_body=await request.body(),
                ingress_signature=ingress_signature,
            )
            return jsonable_encoder(result)

        @app.post("/v1/provider-callbacks/mpesa/timeout", status_code=202)
        async def mpesa_timeout(
            ...
            result = mpesa_callbacks.process_timeout(
                raw_body=await request.body(),
                ingress_signature=ingress_signature,
            )
            return jsonable_encoder(result)
