...
from railone_contracts.models import (
    AccountEndpoint,
    AccountRole,
    require_minor_units,
    require_text,
    ...
    def build_plan(
        ...
        *,
        ...
        ranked = self._scorer.rank(
            ...
            source_institution_id=utt["payer"]["endpoint"]["institution_id"],
            destination_institution_id=utt["beneficiary"]["endpoint"]["institution_id"],
            at=instant,
        )
        if not ranked:
...
class RttAttemptService:
    ...
    def start_next(
        ...
        if not verification.valid:
            raise PermissionError(f"UTT rejected: {verification.reason}")
        utt = signed_utt.payload
        if utt.get("endpoint_model_version") != 1:
            raise PlanNotExecutableError("UTT uses an unsupported endpoint model")
        if self._endpoints is None:
            if not self._allow_unverified:
                raise RuntimeError("partner endpoint validator is required for RTT start")
        else:
            payer = utt["payer"]
            beneficiary = utt["beneficiary"]
            self._endpoints.validate_endpoint(
                actor_id=payer["actor_id"],
                endpoint=AccountEndpoint.from_payload(payer["endpoint"]),
                required_role=AccountRole.DEBIT,
                currency=utt["currency_from"],
            )
            self._endpoints.validate_endpoint(
                actor_id=beneficiary["actor_id"],
                endpoint=AccountEndpoint.from_payload(beneficiary["endpoint"]),
                required_role=AccountRole.CREDIT,
                currency=utt["currency_to"],
            )
    ...
    def resolve_reconciliation(
        ...
        *,
        ...
        """Resolve an RTT previously blocked on an unknown provider outcome."""

        instant = require_aware("reconciliation timestamp", at or datetime.now(timezone.utc))
        cost = require_minor_units("actual_cost_minor", actual_cost_minor, allow_zero=True)
        attempt = self._executions.require_attempt(require_text("rtt_id", rtt_id))
        if attempt.state is not AttemptState.RECONCILIATION_REQUIRED:
            raise PlanNotExecutableError("RTT is not awaiting reconciliation")
        plan = self._executions.require_plan(attempt.plan_id)
        if (
            plan.status is not PlanStatus.RECONCILIATION_REQUIRED
            or plan.current_rtt_id != rtt_id
        ):
            raise PlanNotExecutableError("execution plan is not awaiting this RTT")
        spent = plan.routing_cost_spent_minor + cost
        if succeeded:
            updated_attempt = replace(
                attempt, state=AttemptState.SUCCEEDED,
                actual_cost_minor=cost, updated_at=instant,
            )
            updated_plan = replace(
                plan, routing_cost_spent_minor=spent, current_rtt_id=None,
                previous_rtt_id=rtt_id, previous_route_id=attempt.route_id,
                status=PlanStatus.FINALIZED, successful_route_id=attempt.route_id,
                version=plan.version + 1, updated_at=instant,
            )
        else:
            code = require_text("failure_code", failure_code or "PROVIDER_CONFIRMED_FAILURE").upper()
            if failure_disposition is FailureDisposition.RECONCILIATION_REQUIRED:
                raise ValueError("reconciliation resolution must be success or confirmed failure")
            resolution_failure = RouteFailure(
                rtt_id=rtt_id, route_id=attempt.route_id, failure_code=code,
                disposition=failure_disposition, recorded_at=instant,
            )
            if failure_disposition is FailureDisposition.TERMINAL:
                status = PlanStatus.FAILED
            else:
                affordable = any(
                    route.candidate.route_id in plan.remaining_route_ids
                ...
                status = (
                    ...
                    if plan.attempts_used >= plan.max_attempts or not affordable
                    else PlanStatus.ACTIVE
                )
            updated_attempt = replace(
                attempt, state=AttemptState.FAILED, failure_code=code,
                actual_cost_minor=cost, updated_at=instant,
            )
            updated_plan = replace(
                plan, failures=plan.failures + (resolution_failure,),
                routing_cost_spent_minor=spent, current_rtt_id=None,
                previous_rtt_id=rtt_id, previous_route_id=attempt.route_id,
                status=status, version=plan.version + 1, updated_at=instant,
            )
        self._executions.commit_transition(
            previous_version=plan.version,
            plan=updated_plan,
