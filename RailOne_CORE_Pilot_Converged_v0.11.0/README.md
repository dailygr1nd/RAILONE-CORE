...
This repository is the converged Step 11 pilot baseline. The cumulative Step 10
implementation is promoted to the repository root; superseded step copies and
the legacy prototype are deliberately excluded.
...
The repository contains only test in-memory key providers. A sandbox deployment
must supply isolated signing and continuity-secret services. Private keys,
...
The codebase is an architecture-convergent prototype. It is eligible for a
simulated integration pilot only after CI executes all tests without skips,
including the optional HTTP and disposable PostgreSQL gates. It is not yet a
